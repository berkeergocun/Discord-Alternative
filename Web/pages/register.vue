<template>
  <div class="fixed inset-0 bg-blurple overflow-hidden">
    ...
    <!-- Main content -->
    <div class="relative z-10 h-full flex items-center justify-center p-4 overflow-y-auto">
      <div class="w-full max-w-[480px] my-8">
        ...
        <div class="bg-bg-secondary rounded-lg shadow-2xl p-8">
          <div class="text-center mb-6">
            <h1 class="text-2xl font-bold text-text-primary mb-2">Hesap oluştur</h1>
          </div>
          ...
          <form @submit.prevent="handleRegister" class="space-y-5">
            ...
            <div>
              <label class="block text-xs font-bold text-text-secondary uppercase mb-2">
                Görünen Ad <span class="text-red-400">*</span>
              </label>
              <input
            ...
            <div>
              <label class="block text-xs font-bold text-text-secondary uppercase mb-2">
                Kullanıcı Adı <span class="text-red-400">*</span>
              </label>
              <input
                ...
                title="Kullanıcı adı 3-32 karakter olmalı ve sadece küçük harf, rakam, nokta ve alt çizgi içerebilir"
                required
              />
              <p class="text-xs text-text-muted mt-1">
                Küçük harf, rakam, nokta ve alt çizgi kullanabilirsin
              </p>
            </div>
            ...
            <div>
              ...
              <div class="grid grid-cols-3 gap-3">
                <select
                  ...
                  class="px-3 py-2.5 bg-bg-tertiary border-none rounded text-text-primary focus:outline-none focus:ring-2 focus:ring-blurple transition-all"
                  required
                >
            ...
            <div class="flex items-start gap-3">
              <input
                ...
                type="checkbox"
                id="terms"
                class="mt-1 w-5 h-5 bg-bg-tertiary border-none rounded cursor-pointer accent-blurple"
                required
              />
              <label for="terms" class="text-xs text-text-muted cursor-pointer select-none">
                <span class="text-link hover:underline">Hizmet Koşulları</span> ve 
                <span class="text-link hover:underline">Gizlilik Politikası</span>'nı okudum ve kabul ediyorum
              </label>
            </div>
            ...
            <div class="text-sm text-text-muted mt-2">
              <NuxtLink to="/login" class="text-link hover:underline">
        ...
        <!-- Terms disclaimer -->
        <p class="text-xs text-white/60 text-center mt-6 px-4">
          Kayıt olarak bot değilsin ve en az 13 yaşında olduğunu kabul edersin.
        </p>
      </div>
    </div>
  </div>
...
const handleRegister = async () => {
  ...
  try {
    ...
    if (age < 13) {
      error.value = 'En az 13 yaşında olmalısın'
      loading.value = false
      return
    }
    ...
    if (result.success) {
      // Navigate to channels after successful registration
      navigateTo('/channels/@me')
    } else {
      error.value = result.error || 'Kayıt olurken bir hata oluştu'
    }
  } catch (e: any) {
    error.value = e.message || 'Kayıt olurken bir hata oluştu'
  } finally {
    loading.value = false
  }
}
